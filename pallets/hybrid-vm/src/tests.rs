...
fn test_evm_call_wasm() {
    ...
    ExtBuilder::default().existential_deposit(100).build().execute_with(|| {
        ...
        let bob_balance_before_test =
            <Result<u128, u8> as Decode>::decode(&mut &bob_balance_before[..])
                .unwrap()
                .unwrap();
        ...
        let bob_balance_after_test =
            <Result<u128, u8> as Decode>::decode(&mut &bob_balance_after[..])
                .unwrap()
                .unwrap();
...
fn test_evm_call_wasm_balance() {
    ...
    ExtBuilder::default().existential_deposit(100).build().execute_with(|| {
        ...
        let evm_call_wasm_selector =
            &Keccak256::digest(b"evmCallWasmBalance(bytes20,bytes32)")[0..4];
...
fn test_evm_call_wasm_echo() {
    ...
    ExtBuilder::default().existential_deposit(100).build().execute_with(|| {
        ...
        // let wasm_contract: [u8; 20] = wasm_addr.clone().into();
        let wasm_contract = <mock::Test as pallet::Config>::AccountId32Mapping::id_to_id32(
            wasm_addr.clone().into(),
        );
