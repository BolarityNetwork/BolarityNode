...
mod erc20 {
    ...
    impl Erc20 {
				...
				b if b == transfer_selector => {
					...
					match parameter {
						Ok(t) => {
							...
							let return_result = self.transfer(accountid, balance).is_ok();
        ...
        pub fn wasmCallEvmBalance(
            ...
            let ret = self.env().extension().call_evm_extension(input.as_bytes().to_vec());
			let return_value_offset: usize = match ret.find(r#""ReturnValue":[""#) {
				Some(r) => r,
				None => return Err(Error::Other(String::from("Call EVM error, no ReturnValue!"))),
			};
			#[allow(clippy::arithmetic_side_effects)]
            let result: Balance = match ret[return_value_offset+16..ret.len()-3].parse::<Balance>() {
				Ok(r) => r,
